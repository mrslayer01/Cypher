export class CypherSystemTokenRuler extends foundry.canvas.placeables.tokens.TokenRuler {
  static WAYPOINT_LABEL_TEMPLATE = "systems/cypher/templates/actor/apps/token-ruler-label.hbs";
  _getWaypointLabelContext(waypoint, state) {
    const isGridless = this.token.scene.grid.type == 0;
    const rulerSetting = game.settings.get("cypher", "showRulerGridless");
    const isPathfinding =
      game.user.getFlag("cypher", "isPathfinding") ||
    ...
    if (category.label) {
      context.cyphersystemLabel = game.i18n.format("CYPHER.DistanceLabelCategory", {
        category: category.label,
        distance: category.cost,
        unit: unit
      });
    } else {
      context.cyphersystemLabel = game.i18n.format("CYPHER.DistanceLabel", {
        distance: category.cost,
        unit: unit
      });
    }
...
function getCategory(token, cost, unit) {
  ...
  // Set meters or feet
  if (["m", "meter", "metre", game.i18n.format("CYPHER.UnitDistanceMeter")].includes(unit)) {
    rounding = 0.5;
    immediate = 3;
    short = 15;
    long = 30;
    veryLong = 150;
  } else if (["ft", game.i18n.format("CYPHER.UnitDistanceFeet")].includes(unit)) {
    rounding = 1;
    immediate = 10;
    short = 50;
    long = 100;
  ...
  // Define range bands
  if (cost <= immediate) {
    label = game.i18n.format("CYPHER.Immediate");
    color = 0x0000ff;
  } else if (cost <= short) {
    label = game.i18n.format("CYPHER.Short");
    color = 0x008000;
  } else if (cost <= long) {
    label = game.i18n.format("CYPHER.Long");
    color = 0xffa500;
  } else if (cost <= veryLong) {
    label = game.i18n.format("CYPHER.VeryLong");
    color = 0xff0000;
  }
...
export class CypherSystemToken extends foundry.canvas.placeables.Token {
  _initializeRuler() {
    ...
    const excludedActorTypesString = game.settings.get("cypher", "disableRulerTypes");
  ...
  async _addDragWaypoint(point, { snap = false } = {}) {
    ...
    await game.user.setFlag("cypher", "isPathfinding", true);
  }

  async _triggerDragLeftCancel() {
    super._triggerDragLeftCancel();
    await game.user.setFlag("cypher", "isPathfinding", false);
  }
}
